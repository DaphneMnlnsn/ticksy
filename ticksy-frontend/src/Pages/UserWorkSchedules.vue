                            ...
                            <div class="right-side" v-if="selectedSchedule">
                                <div class="container">
                                    ...
                                    <div class="mini-card">
                                        ...
                                        <div v-if="selectedTeamName" class="team-name">{{ selectedTeamName }}</div>
                                        <div class="row2">
                                            <div class="avatar-group">
                                                <img v-for="member in selectedTeamMembers"
                                                    :key="member.userId"
                                                    :src="member.avatarUrl"
                                                    :alt="member.fullName"
                                                    @mouseenter="showTeamTooltip($event, member)"
                                                    @mousemove="moveTeamTooltip"
                                                    @mouseleave="hideTeamTooltip"
                                                    class="avatar"
                                                />
                                                
                                                <span v-if="!selectedTeamMembers?.length" class="no-members">
                                                    None
                                                </span>
                                            </div>
                                        </div>
                                            ...
                                            <tr>
                                                ...
                                                <th><div class="table-header">
                                                    <template v-if="selectedRequests.length > 0">
                                                        <div class="dropdown-item-multiple reject" @click="bulkCancel" title="Cancel Selected">
                                                            <CircleX size="18" />
                                                        </div>
                                                    </template>
                                                </div></th>
                                                ...
                                                <td>
                                                    <div class="actions" @click.stop>
                                                        ...
                                                        <div v-if="activeMenu === request.id" class="dropdown">
                                                            ...
                                                            <div class="dropdown-item reject" @click="handleCancelRequest(request.id)">
                                                                <CircleX size="16" />
                                                                Cancel
                                                            </div>  
                                                        </div>
...
<script setup lang="ts">
    ...
    import { CheckCircle, CircleX, FilePlus } from 'lucide-vue-next'
    import { computed } from 'vue';
    import { useSearch } from '../services/search'
    import { getMySchedule, getScheduleById, getSchedules } from '../services/schedule'
    import { approveRequest, cancelRequest, getMyRequests, getRequests, rejectRequest } from '../services/timeOff'
    ...
    import { formatDate, formatDuration, formatRange, formatTime } from '../services/formatting'
...
import PolicyListPanel from '../components/PolicyListPanel.vue'
    import { getTeams, getTeam } from '../services/people'
    ...
    const selectedTeamName = computed(() => {
        const selected = teams.value.find(team => team.id === selectedTeamId.value);
    ...
    async function loadSchedules() {
        try {
            ...
            if (data) {
                ...
                activeScheduleId.value = data.id || null;
            }
        } catch (error) {
    ...
    async function loadTeamMembers(teamId: number) {
        try {
            ...
            selectedTeamMembers.value = teamData.members || [];
        } catch (error) {
    ...
    async function bulkCancel() {
        ...
        if (!confirm(`Cancel ${selectedRequests.value.length} items?`)) return;

        try {
            await Promise.all(selectedRequests.value.map(id => cancelRequest(id)));
    ...
    async function handleCancelRequest(id) {
        try {
            ...
            const errorMsg = err.response?.data || "Failed to cancel request";
    ...
    const { search: requestSearch, filtered: filteredRequests } = useSearch(requests, ['type', 'reason']);
